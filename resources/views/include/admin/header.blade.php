                ...
                <ul class="navbar-nav ms-auto">
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link" data-bs-toggle="dropdown" href="#">
                            ...
                            <span class="navbar-badge badge text-bg-warning">{{ count(auth()->user()->notifications) }}</span>
                        </a>
                        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-end">
                            <span class="dropdown-item dropdown-header">{{ count(auth()->user()->notifications) }} Notifications</span>
                            <div class="dropdown-divider"></div>
                            @forelse (auth()->user()->notifications as $notification)
                            <a href="{{ route('admin.order.detail',$notification->data['id']) }}" class="dropdown-item">
                                <i class="bi bi-envelope me-2"></i> Order {{ $notification->data['full_name'] }}
                                <span class="float-end text-secondary fs-7">View Details</span>
                            </a>
                            @empty
                            @endforelse
                            {{-- <div class="dropdown-divider"></div>
